...
def train(dataset_path: str,
          epochs: int, 
          style_image: str,
          trained_model_path: OutputPath(str),
          image_size: int = 224,
          batch_size: int = 4,
         ):
    import numpy as np
...
def infer(
         trained_model_path: InputPath(str),
         content_img_path: str,
         output_image_path: OutputPath(str),
         ):
    import numpy as np
    import torch
    from torch.optim import Adam
    ...
    if dummy:
      ...
      content_image = load_image(content_img_path, scale=5)
    ...
    logger.info(f"{trained_model_path}")
    logger.info(f"{output_image_path}")
    #os.system(f"rm -r {output_image_path}")	
    os.system(f"mkdir -p {output_image_path}")
    ...
    os.system(f"cp {output_image_path} static/")
    save_image("static/out.jpg", output[0])
    print(os.listdir('./static/'))
    app = Flask(__name__)
    @app.route('/')
    def index():
        return send_file(output_image_path, mimetype='image/jpg')
        #return f'<img src="/static/out.jpg" alt="User Image">'
...
infer_op = create_component_from_func(
                    func = infer,
                    base_image="jy3694/kubeflow_component_1:latest",
                    packages_to_install=["loguru", "flask"]
                )
...
def my_pipeline(dataset_path: str="/data/coco_sampled", 
    ...
    trained_model = train_op(dataset_path,
                            epochs,
                            style_image,)
    infer_output = infer_op(trained_model.output, 
